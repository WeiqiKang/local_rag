...
def get_chat_block():
    with gr.Blocks(theme=gr.themes.Base(),css=".gradio_container { background-color: #f0f0f0; }") as chat:
        ...
        with gr.Row():     
            ...
            with gr.Column(scale=5):
                ...
                with gr.Accordion(label="模型设置",open=True):
                    model =gr.Dropdown(choices=['deepseek-v3','qwq-32b'],label="选择模型",interactive=True,value="deepseek-v3",scale=2)
                    role = gr.Dropdown(choices=['数学解题专家','数学讲题老师', "克拉时光机客服"],label="选择角色",interactive=True,value="数学解题专家",scale=2)
        ...
        input_message.submit(fn=user,inputs=[input_message,chatbot],outputs=[input_message,chatbot],queue=False).then(
            fn=get_model_response,inputs=[input_message,chatbot,model,role, temperature,max_tokens,history_round,knowledge_base,similarity_threshold,chunk_cnt],outputs=[chatbot,chunk_text]
            )
